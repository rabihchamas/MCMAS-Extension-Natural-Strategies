...
for (map< string, bool_expression * >::iterator i = is_evaluation->begin();
  ...
  int natsize = 0;
  int ddd;
 // natsize = cmplxNatStr3(phi1,res, name, para);
  //cout<<" Number of atomic propositions is "<<atmprops.size()<<endl;
  //cout<< "The global natural strategy is "<<endl;
  ddd = cmplxNatStr2(phi1,res, name, para);
  cout<<" the complexity of natural strategy is "<<ddd<<endl<<endl;


/*
bool_expression *simple1 = is_evaluation->find("p1")->second;
BDD P1 = simple1->encode_bdd_flat(para, para->bddmgr->bddOne());
bool_expression *simple2 = is_evaluation->find("p2")->second;
BDD P2 = simple2->encode_bdd_flat(para, para->bddmgr->bddOne());
vector<BDD> results = {P1,P2};
vector<BDD> *res = &results;
  int natsize = 0;
  natsize = cmplxNatStr3( phi1,res, name, para);
  */
//resultnat =  vectNat(phi1, name, para);
//cout<< " we have "<<resultnat.size()<<" guarded actions "<<endl;

 //resultnat =  vectNat(para->bddmgr->bddOne(), phi1, name, para);

 // cout <<endl<<  " The complexity of natural strategy is "<< natsize << endl;
...
vector<BDD> results = {P1,P2,P3,P4};
vector<BDD> *res = &results;



cout<<"The test of "
  int aa =  Cudd_SupportSize(para->bddmgr->getManager(), result.getNode());
  vector<BDD> * w = para->v;
  for (unsigned int j = 0; j < w->size(); j++){
   ...
   cout <<  "The complexity of the formula is "<< aa << endl; */
/*******************EndTesting*****************************/
